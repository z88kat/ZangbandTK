...
def source_of(path: Path) -> str:
    """A generated file's source, written relative to the repository root.

    These lines are committed, so an absolute path makes the output depend on
    where the tree happens to sit: two people regenerating the same data get
    different files, and "the shipped file is what the generator produces"
    stops being something anyone can check. Relative to the root it is the
    same everywhere, which is what makes the check in
    `scripts/check-build-lists` possible at all.
    """
    ...
    if args.write:
        ...
        aformat.write(
            ...
            preamble=(
                ...
                f"# Source: {source_of(ZANGBAND / 'r_info.txt')}\n"
            ),
        )
...
def cmd_artifacts(args) -> int:
    ...
    for key, rec in sorted(candidates, key=lambda kv: kv[1].index):
        ...
        #
        # The USE hook only. `a_info.txt` hangs eight different Lua hooks off
        # `L:` -- USE, HIT, MAKE, BONUS, TIMED, DESC, SPOIL, ALTER -- and
        # joining them fed `match_activation()` scripts that are not
        # activations at all. A HIT hook is what the weapon does when it
        # strikes, and it calls the same functions an activation would, so a
        # blade that confused on hit read as a blade that could be *activated*
        # for confusion. Whichever pattern sat earliest in the table won,
        # which is how eleven artifacts lost the activation their USE hook
        # actually described and one gained an activation it never had.
        #
        # The ego and kind path already reads `USE` alone (see the
        # `rec.all("L")` filter further down); this is the same rule, applied
        # where it was missed.
        script = "\n".join(
            line.partition(":")[2]
    ...
    preamble = (
        ...
        "(BAL-12).\n"
        f"# Source: {source_of(ZANGBAND / 'a_info.txt')}\n"
    )

    if args.check:
        # Does the committed file still say what this converter produces?
        #
        # `realms` and `mutations` have had this and `artifacts` had not, and
        # the gap cost the same kind of thing twice over: the activation table
        # was matched against every Lua hook rather than the USE hook alone, so
        # `of Hunger` carried an activation derived from its regeneration
        # timer, and eleven artifacts whose USE hook called a primitive the
        # table did not list imported with no activation at all. Neither shows
        # up as a parse error -- an artifact without an activation is a legal
        # artifact -- and nothing compared the committed file with the
        # converter for months.
        made = aformat.render(entries, preamble=preamble)
        have = (GAMEDATA / "artifact.zangband.txt").read_text(encoding="utf-8")

        if made != have:
            import difflib

            print("artifact data check")
            print("=" * 72)
            for line in list(difflib.unified_diff(
                    have.splitlines(), made.splitlines(),
                    "lib/gamedata/artifact.zangband.txt", "converter output",
                    lineterm=""))[:40]:
                print("  %s" % line)
            print()
            print("  FAILED -- lib/gamedata/artifact.zangband.txt is not what "
                  "the converter produces")
            return 1
        print("artifact data check: lib/gamedata matches the converter")
...
def _mutation_powers() -> tuple[dict, dict]:
    ...
    entries = [(code, spec, "power-") for code, spec in table.items()
               if code not in ("random", "continuous")]
    entries += [(code, spec, "fires-")
                for code, spec in table.get("random", {}).items()]

    # `[continuous.X]` sections emit nothing at all. A continuous mutation is
    # normally expressible as `values:` and `flags:` in mutation.txt and needs
    # no entry here; the two that are listed are read by code inside other
    # systems instead, so what is recorded is the disposition and not a
    # translation. They belong in the report for the same reason every other
    # disposition does -- a mutation that does nothing and a mutation whose
    # effects the converter dropped look identical from the data.
    for code, spec in table.get("continuous", {}).items():
        if "reject" in spec:
    ...
    for code, spec, prefix in entries:
        ...
        for band in bands:
            ...
            for item in band["effects"]:
                ...
                elif item.startswith("msg:"):
                    # `power-effect-msg`, not `power-effect:msg:` -- the
                    # parser registers it as its own directive
                    # (init.c:5059), and the other spelling reads "msg" as
                    # the effect's name and refuses the file. The committed
                    # data has always had it right and this had always
                    # emitted it wrong, so regenerating mutation.txt
                    # produced a file the game would not load. Nothing
                    # caught it because `mutations` has no --check.
                    out.append(prefix + "effect-msg:" + item[len("msg:"):])
                else:
...
def cmd_mutations(args) -> int:
    ...
    if args.check:
        # Does the committed file still say what this converter produces?
        #
        # `realms` has had this since 3.64.0 and `mutations` had not, and the
        # gap cost something real: the emitter wrote `power-effect:msg:` where
        # the parser registers `power-effect-msg` (init.c:5059), so
        # regenerating mutation.txt produced a file the game would refuse to
        # load. The committed data had been hand-corrected and the converter
        # never was, and nothing compared the two for three months.
        made = aformat.render(entries, preamble=_MUT_PREAMBLE)
        have = (GAMEDATA / "mutation.txt").read_text(encoding="utf-8")

        if made != have:
            import difflib

            print("mutation data check")
            print("=" * 72)
            for line in list(difflib.unified_diff(
                    have.splitlines(), made.splitlines(),
                    "lib/gamedata/mutation.txt", "converter output",
                    lineterm=""))[:40]:
                print("  %s" % line)
            print()
            print("  FAILED -- lib/gamedata/mutation.txt is not what the "
                  "converter produces")
            return 1

        print("mutation data check: lib/gamedata/mutation.txt reproduces "
...
def main() -> int:
    ...
    arts.add_argument("--check", action="store_true",
                      help="fail if lib/gamedata disagrees with the converter")
    ...
    mutations.add_argument("--write", action="store_true",
                           help="write the data file as well as the report")
    mutations.add_argument("--check", action="store_true",
                           help="verify lib/gamedata/mutation.txt reproduces")

...
**Eleven mutations do nothing**, all deferred and none refused — DEC-52 reversed the one
rejection — recorded per-mutation in `tools/zconv/mutmap.toml` and listed in
`docs/mutations.rst`: seven activatable and four random. Every one needs machinery 4.2 has not got — pets, an incorporeal player state, an

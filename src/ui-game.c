...
struct cmd_info cmd_info[] =
{
	...
	/*
	 * The second key is the roguelike keyset's.  Both of these commands want a
	 * letter and cannot have one: J and N are two of the eight running keys
	 * (BHJKLNUY) that keyset claims, and by 4.2 every other letter and every
	 * usable control key is spoken for in one keyset or the other.  So the
	 * roguelike bindings are symbols.  Without them key[1] would be 0 and these

 ...
 */
/**
 * The player block, shared between its versions (ZangbandTK).
 *
 * Version 2 added the Lord of Chaos the character serves.  Written as one body
 * with a flag rather than two copies -- which is what this was at first, and
 * which meant every future change to the block had to be made twice with
						 ...
						 bool with_pet_orders)
{
	 ...
	 * Saves written before PLR-05 have no such field at all.
	 */
	player->patron = NULL;
	if (with_patron) {
		rd_string(buf, sizeof(buf));
...
int rd_chunks(void)
{
	...
	for (j = 0; j < chunk_max; j++) {
		...
		if (OPT(player, birth_levels_persist)) {
			...
			rd_string(buf, sizeof(buf));
			string_free(c->name);
			c->name = string_make(migrate_level_name(buf));
			rd_s32b(&c->turn);
			rd_u16b(&tmp16u);
			c->depth = tmp16u;
			rd_byte(&c->feeling);
